 ...
 * すべての項目を記載しないと登録できません。
 ...
 # 工夫したポイント
 トップページの並び順を食材を届けた数が多い順番にして並べたこと。
 
 # 今後実装したい機能
 今後実装したい機能としては、それぞれの駅周辺のお店でグループを作り、お客側は駅を検索
 トップページの並び順が上位にあるお店はクーポン券などを使えるようにして食品ロスに貢献したお店は有利な条件にする。
 お店同士で
 トップページの並び順が上位にあるお店はクーポン券などを使えるようにして食品ロスに貢献したお店は有利な条件にする。
 お店がお客にグループのお店を２軒目などに勧めて、お店同士の結びつきを強くする
 
 # 課題
 今のままでは、お店だけではなく一般のユーザーも登録できてしまうので、新規登録に店名、メールアドレス、パスワードの他に、運営側からの会員番号みたいなものを発行してそれがないと、新規登録できないようにする
 課題としては、産地が違う食材や、同じ食材を使ってない場合はお店の味などに問題が出てしまう可能性があるので、なるべく同じ食材を使うように、農家の人から直接仕入れる。そうすることによって、同じ食材を使っているから、先ほどの問題も解消されるし、生産者側からも、継続的に仕入れてくれるお店があると安定する。
